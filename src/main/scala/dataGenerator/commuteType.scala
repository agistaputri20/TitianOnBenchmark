...
object commuteType {
  ...
  def main(args: Array[String]): Unit = {
    val partitions = 1
    val dataper = 10000
    val name = "commuteType_10000_1"
    val seed = Random.nextLong()
    Random.setSeed(seed)

    val sparkConf = new SparkConf()
    val datasets = Array(
      ("ds1", "src/resources/dataCommuteType")
    )
    ...
    datasets.foreach { case (_, "src/resources/dataCommuteType") =>
      ...
      }.saveAsTextFile("src/resources/dataCommuteType")
    }
  }
}
